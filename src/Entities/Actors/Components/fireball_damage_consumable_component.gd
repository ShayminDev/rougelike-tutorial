...
func activate(action: ItemAction) -> bool:
	...
	if targets.is_empty():
		MessageLog.send_message("There are no targets in radius.", GameColors.IMPOSSIBLE)
		return false
	...
	for target in targets: 
		MessageLog.send_message("The %s is englufed in a fiery explosion, taking %d damage!" % [target.get_entity_name(), damage], GameColors.PLAYER_ATTACK)
